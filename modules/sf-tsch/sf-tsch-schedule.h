...
/**
 * @brief   TSCH schedule data slot type definition.
 */
typedef enum
{
    E_SF_TSCH_SCHEDULE_DATA_SLOTS_ALL = 0,
    E_SF_TSCH_SCHEDULE_DATA_SLOTS_RX,
    E_SF_TSCH_SCHEDULE_DATA_SLOTS_TX
} e_sf_tsch_schedule_data_slot_types_t;
 ...
 *			Devices also add the data slots after a successfull join.
 *
 * @param	addr 	        Address to add data for.
 * @param dataSlotType  Type of data slot to add.
 *
 * @return	0 on success.
 */
 ...
 *			are device specific the according address is required. Gateway
 *			and devices remove data slots whenever the connection was
 *			closed or broke.
 *
 * @param addr          Address to delete data slot for.
 * @param dataSlotType  Type of data slot to delete.
 *
 * @return	0 on success.
 */
