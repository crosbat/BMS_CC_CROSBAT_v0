...
/*----------------------------------------------------------------------------*/
/*! sf_absoluteTime_setTimeBase */
/*----------------------------------------------------------------------------*/
E_SF_RETURN_t sf_absoluteTime_setTimeBase(uint32_t timestamp)
{
  uint64_t lastEbAbsoluteTime = tsch_get_last_eb_timestamp();
  uint32_t timeOffsetSec = 0;
  uint64_t currentTime = RTIMER_NOW();
  uint32_t absoluteTime = 0;

  if(TSCH_BEACON_SEND_PERIOD >= timestamp)
  {
    return E_SF_ERROR_INVALID_PARAM;
  }

  /* Calculate internal time and current time difference */
  if(0U != lastEbAbsoluteTime)
  {
    if(currentTime > lastEbAbsoluteTime)
    {
      timeOffsetSec = (currentTime - lastEbAbsoluteTime) / RTIMER_SECOND;
      /* Check if the offset is greater than beacon period. */
      if(timeOffsetSec > TSCH_BEACON_SEND_PERIOD)
      {
        timeOffsetSec = TSCH_BEACON_SEND_PERIOD;
      }
    }
  }

  /* Subtract time offset to set correct time for next beacon */
  absoluteTime = timestamp - timeOffsetSec;
  tsch_set_internal_absolute_time(absoluteTime);
